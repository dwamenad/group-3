...
g25datadir=/ZPOOL/data/projects/g25
...
DATA=${g25datadir}/derivatives/fmriprep/sub-${sub}/func/sub-${sub}_task-${TASK}_run-${run}_part-mag_space-MNI152NLin6Asym_desc-preproc_bold.nii.gz

CONFOUNDEVS=${g25datadir}/derivatives/fsl/confounds_tedana/sub-${sub}/sub-${sub}_task-${TASK}_run-${run}_desc-TedanaPlusConfounds.tsv
...
GO_INCORRECT=${EVDIR}_go_incorrect.txt
if [ -e $GO_INCORRECT ]; then
    MODEL="1a"
else
    MODEL="1b"
fi
...
if [ "$ppi" == "ecn" -o  "$ppi" == "dmn" ]; then
	...
	OUTPUT=${MAINOUTPUT}/L1_task-${TASK}_model-${MODEL}_type-melodic-nppi-${ppi}_run-${run}_sm-${sm}
	...
	for net in `seq 0 9`; do
		# Change file to reslice for line 69 -- do later
	...
	if [ "$ppi" == "0" ]; then
		...
		# Change file in 141 to reslice mask  
